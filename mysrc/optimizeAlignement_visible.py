...
def img2da4residu(rrh, atr, da1Ref, flag_ref=False):
    ...
    da1 = da1.coarsen(dim={'x': 32, 'y': 32}, boundary="trim").mean()
...
def residual(args, *params):
    ...
    imuNcOoGeojson.imutogeojson( params[-2], wkdir, indirimg, flightname, correction_xyz, correction_opk, src_files,str_tag=str_tag)
    ...
    for idimg,da1Ref in zip(idimgs,da1Refs):
        ...
        resi += float(abs((da1 - da1Ref)).sum()) #.where(overlap_mask,0.0).sum())
       
        #da1_mask = (da1 > 0)  # or any valid domain definition
        #resi += float(np.abs(da1_mask.where(~overlap_mask,0.0).sum())) * penalty_factor
...
##################################
    ...
    intparamFile = f"{indir}/io/{flightname}_int_opencv_param.yaml"
    ...
    if False: 
        ...
        popt = np.load('resbrute1_xycopk_minimize.npy',allow_pickle=True).item().x
        ...
        params = [ {'offset':offset,'scale':scale}, True,'xyzopk', imu, [0.5,0.5,0.5] ]
    ...
    xc,yc,zc,oc,pc,kc = 0.5,0.5,0.5,  .24,0.5,0.5
    
    resbrutef =  [xc,yc,zc] + [oc,pc,kc]
    #resbrutef =  [oc,pc,kc]
    
    from scipy.optimize import minimize
    params = [ {'offset':offset,'scale':scale}, False,'xyzopk', imu, [0.5,0.5,0.5] ]
